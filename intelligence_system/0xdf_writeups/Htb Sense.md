...
date: 2021-03-11T10:00:00+00:00
difficulty: Easy [20]
tags: htb-sense, hackthebox, ctf, pfsense, nmap, gobuster, dirbuster, searchsploit, metasploit, command-injection, feroxbuster, cve-2016-10709, burp, oscp-like-v2, oscp-like-v1
---

![Sense](https://0xdfimages.gitlab.io/img/sense-cover.png)
...
| Name | [Sense](https://hackthebox.com/machines/sense)  [Sense](https://hackthebox.com/machines/sense) [Play on HackTheBox](https://hackthebox.com/machines/sense) |
| --- | --- |
| Release Date | 21 Oct 2017 |
| Retire Date | 24 Mar 2018 |
| OS | FreeBSD FreeBSD |
| Base Points | Easy [20] |
| Rated Difficulty | Rated difficulty for Sense |
| Radar Graph | Radar chart for Sense |
| First Blood User | 02:53:47[echthros echthros](https://app.hackthebox.com/users/2846) |
| First Blood Root | 02:47:11[echthros echthros](https://app.hackthebox.com/users/2846) |
...
I hadn’t figure out my go-to syntax for `nmap` back then (first `-p- --min-rate 10000`, then `-p [ports] -sCV`). The web server is lighttpd. The certificate on TLS isn’t filled in with any information. I can look at it in Firefox and confirm it is just defaults:

[![image-20210310163104815](https://0xdfimages.gitlab.io/img/image-20210310163104815.png)](https://0xdfimages.gitlab.io/img/image-20210310163104815.png)

[*Click for full image*](https://0xdfimages.gitlab.io/img/image-20210310163104815.png)
...
![Image](https://0xdfimages.gitlab.io/img/login.png)
...
I switched to another tool, `dirbuster`, because in 2017, that was the best tool to recurrsively directory brute force, and I wanted to run a much more extensive brute force, looking in subdirectories and for more extensions. I kicked it off with the following:
...
![Dirbuster](https://0xdfimages.gitlab.io/img/dirbuster.png)
...
![image-20210311101318041](https://0xdfimages.gitlab.io/img/image-20210311101318041.png)
...
![image-20210311101512285](https://0xdfimages.gitlab.io/img/image-20210311101512285.png)
...
![RRDGraphs](https://0xdfimages.gitlab.io/img/RRDGraphs.png)
...
I’ve never really been a master at `meterpreter`, so I just dropped into a shell, and used Python to get a PTY:
...
I won’t do the other half of the shell upgrade (`bg`, `stty raw -echo`, `fg`, `reset`), as that doesn’t work inside Metasploit. I can grab both flags:
...
I rarely go looking for `.txt` files in directory brute forces on HTB anymore, but that was much more common in older machines. I wouldn’t run `dirbuster` today, but rather perhaps either more targeted `gobuster` (I didn’t actually need the recurrsion), or [feroxbuster](https://github.com/epi052/feroxbuster), a tool I’ve only recently started playing with that looks quite slick. For this example:
...
Some Googling today for the CVE (always in quotes like `"CVE-2016-10709"` or Google will return others) led to [this page] which contains a writeup of the exploit.
...
1. GET `index.php` to get the CSRF token.
2. POST `index.php` to login using the provided creds, and the CSRF from 1.
3. GET `index.php` following the 302 redirect on successful login in 2.
4. GET `status_rrd_graph_img.php` with a long payload (mostly in octal) in the `graph` parameter.
5. GET `status_rrd_graph_img.php` with a short payload in `graph`.
...
So there’s a command running on the PFSence host that includes this, and the pipe sends the output (which I don’t care about) into the `printf` which ignores that output, prints the encoded stuff into `sh`. To figure out what it’s doing, I’ll just run the `printf`:
...
So it’s using `echo` to print a bunch of PHP code into a file, `RCMnpqd`. I bet that code is the meterpreter stager.
...
It’s running the previous file with `php` to execute it.

If I wanted to make my own payload, the trick looks to be encoding it as octal (base-8), and then using `printf` to send it to `sh|echo`.
